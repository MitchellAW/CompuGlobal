...
class CompuGlobalAPITestCase(unittest.TestCase):
    ...
    def test_view_episode_1(self):
        ...
        self.assertEqual(FRINKIAC.view_episode('S11E10', 339960, 0),
                         frinkiac_data['episode_test_1'])
        self.assertEqual(FRINKIAC.view_episode('S11E10', 339960, 349960),
                         frinkiac_data['episode_test_2'])

    def test_view_episode_2(self):
        """Tests ``api/episode/{episode}/{start}/{end}`` endpoint with
        invalid parameters"""
        self.assertEqual(FRINKIAC.view_episode('Invalid', 0, 0),
                         frinkiac_data['episode_test_3'])

    def test_get_frames_1(self):
        """Tests ``api/frames/{episode}/{timestamp}/{before}/{after}``
        endpoint"""
        self.assertEqual(frames_to_json(
            FRINKIAC.get_frames('S02E01', 232973, 5000, 5000)),
                         frinkiac_data['frames_test_1'])
        self.assertEqual(frames_to_json(
            FRINKIAC.get_frames('S07E07', 225507, 3000, 3000)),
                         frinkiac_data['frames_test_2'])
    ...
    def test_get_nearby_frames_1(self):
        ...
        self.assertEqual(frames_to_json(
            FRINKIAC.get_nearby_frames('S07E10', 498797)),
                         frinkiac_data['nearby_frames_test_1'])
        self.assertEqual(frames_to_json(
            FRINKIAC.get_nearby_frames('S03E17', 1314891)),
                         frinkiac_data['nearby_frames_test_2'])
    ...
    def test_search_1(self):
        ...
        self.assertEqual(frames_to_json(
            FRINKIAC.search('stupid sexy flanders')),
            frinkiac_data['search_test_1'])
        self.assertEqual(frames_to_json(
            FRINKIAC.search('i sleep in a racing car')),
            frinkiac_data['search_test_2'])
    ...
    def test_search_for_screencap_1(self):
        ...
        json data"""
        self.assertEqual(FRINKIAC.search_for_screencap(
            'stupid sexy flanders').json, frinkiac_data['test_1'])
        self.assertEqual(FRINKIAC.search_for_screencap(
            'i sleep in a racing car').json, frinkiac_data['search_test_3'])
